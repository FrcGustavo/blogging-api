...
findStub
  .withArgs({ isPublic: true, isDisabled: false })
  .returns({ limit: (query) => limitStub(query) });

countDocumentsStub
  .withArgs({ isPublic: true, isActive: true })
  .resolves(PostsMock.length);
createStub
  .withArgs({
    title: '',
    description: '',
...
updateOneStub
  .withArgs({ slug: PostsMock[0].slug }, { isActive: false })
  .resolves({ nModified: 1 });
updateOneStub
  .withArgs({ slug: 'error' }, { isActive: false })
  .resolves({ nModified: 1 });
updateOneStub
  .withArgs({ slug: PostsMock[0].slug }, { title: 'this is a new title' })
  .resolves({ nModified: 1 });
updateOneStub
  .withArgs({ slug: 'error' }, { title: 'this is a new title' })
  .resolves({ nModified: 1 });
findOneStub
  .withArgs({
